...
int index, orig_end;
  ...
  if (list_is_in_by_size(freelist, blocksize)) {
      if((index = list_get_index_of_by_Size(freelist, blocksize)) >= 0) {
           ...
           // add block to allocat list, but modify the end 
           ...
           frag = malloc(sizeof(block_t));           
           frag->pid = 0;
           frag->start = blk->end + 1;
           frag->end = orig_end;
        
           switch(policy) {
             case 1:  list_add_to_back(freelist, frag);   // FIFO Order
                      break;
             case 2:  list_add_ascending_by_blocksize(freelist, frag);   // BESTFIT Order
                      break;
             case 3:  list_add_descending_by_blocksize(freelist, frag);  // WORSTFIT Order
                      break;
           }
...
int index;
 if (list_is_in_by_pid(alloclist, pid)) {
      if((index = list_get_index_of_by_Pid(alloclist, pid)) >= 0) {
           blk =  list_remove_at_index(alloclist, index);   // remove block from alloclist
        
           // add block to free list, but modify the pid

           blk->pid = 0;          
           switch(policy) {
             case 1:  list_add_to_back(freelist, blk);   // FIFO Order
                      break;
             case 2:  list_add_ascending_by_blocksize(freelist, blk);   // BESTFIT Order
                      break;
             case 3:  list_add_descending_by_blocksize(freelist, blk);  // WORSTFIT Order
                      break;
           }
      }
      else {
